...
Step-by-step guide to host the **frontend** (React/Vite app) on Vercel. The **backend** (Flask API) must be hosted elsewhere (Render, Railway, Fly.io, or a VPS)—see [SETUP_OPENFINDVIEW_DOMAIN.md](./SETUP_OPENFINDVIEW_DOMAIN.md) for backend options.
...
3. **Backend URL (for production)**  
   ...
   - `https://api.openfindview.com` (once you point that domain to your backend), or  
...
5. **(Optional) Google sign-in**  
   If you want “Sign in with Google” on the deployed app, you need a Google OAuth Client ID and to add your Vercel URL (and/or `https://app.openfindview.com`) to “Authorized JavaScript origins” in Google Cloud Console.
...
|------|--------|--------------|----------|
| `VITE_BACKEND_URL` | Your backend URL, e.g. `https://api.openfindview.com` or `https://your-app.onrender.com` | Production (and Preview if you want) | Yes |
...
## Step 5: Add a custom domain (e.g. app.openfindview.com)

1. In Vercel, open your project → **Settings** → **Domains**.
2. Click **Add** and enter e.g. **`app.openfindview.com`**.
3. Vercel will show DNS instructions. At your domain registrar (where you bought openfindview.com):
   - Add a **CNAME** record:  
     **Name/host:** `app`  
...
After that, the app is available at **`https://app.openfindview.com`**.

**Backend CORS:** Your backend must allow the frontend origin. This repo’s `backend/config.py` already includes `https://app.openfindview.com`. If you test with the default Vercel URL (e.g. `https://your-project.vercel.app`) before adding a custom domain, add that exact URL to `CORS_ORIGINS` in `backend/config.py`, or the browser will block API requests. After you use a custom domain, that domain is already in CORS.
...
- [ ] Backend is deployed and reachable; CORS allows your Vercel URL and/or `https://app.openfindview.com`.
...
| Google sign-in fails | Add the exact origin (e.g. `https://app.openfindview.com`) to “Authorized JavaScript origins” in Google Cloud Console; no trailing slash. |
...
For full production setup (backend, DNS, cookies, Word preview), see [SETUP_OPENFINDVIEW_DOMAIN.md](./SETUP_OPENFINDVIEW_DOMAIN.md).
