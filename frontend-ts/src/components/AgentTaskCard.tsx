  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: task.status === 'error' ? '1px solid #FECACA' : task.status === 'complete' ? '1px solid rgba(22, 163, 74, 0.5)' : '1px solid #E5E7EB',
        boxShadow: task.status === 'complete' ? 'none' : '0 1px 3px rgba(0,0,0,0.04)',
        cursor: task.status === 'complete' ? 'pointer' : 'default',
        transition: 'border-color 0.15s, box-shadow 0.15s, background-color 0.15s',
        ...(task.status === 'complete' && isHovered ? { backgroundColor: '#FFFFFF', borderColor: 'rgba(22, 163, 74, 0.7)', boxShadow: 'none' } : {}),
        overflow: 'hidden',
        minWidth: 200,
      }}
