...
const extractMarkdownBlocks = (combined: string): { completeBlocks: string[], remainingBuffer: string } => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isLongEnough = line.trim().length > 24 && line.match(/\s/);
    const isMediumLine = line.trim().length >= 10; // Emit sooner so streaming feels responsive (was 16)
    const isShortLineEmit = line.trim().length >= 6; // Emit even short lines at word boundary to avoid hang
    
    // Determine if we should try to emit this block
    // CRITICAL: Only emit if at word boundary to prevent mid-word splits
    const shouldTryEmit = endsAtWordBoundary && (!isLastLine || isHeading || endsWithPunctuation || isBlankLine || isLongEnough || isMediumLine || isShortLineEmit);
...
const OrangeCitationSwoopHighlight: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className="orange-citation-swoop cited-highlight-formatting">
    <style>{`
...
const BlueCitedTextHighlight: React.FC<{
  ...
  onBlueMounted?: (num: string) => void;
}> = ({ children, citationNumber, skipAnimation = false, onBlueMounted }) => {
  React.useEffect(() => {
    if (!skipAnimation && citationNumber != null && onBlueMounted) {
      const t = setTimeout(() => onBlueMounted(citationNumber), 180);
...
// Current citation in review: no tint (white container from CitedTextContainer only). Keeps formatting class for strong/em.
const GreenCitedTextHighlight: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span
...
// Wraps cited text (green/blue/orange highlight) so the response text is not duplicated in the callout. No white box — inline only.
...
const StreamingResponseText: React.FC<{
  ...
  greenCitationNumbers?: Set<string>; // Citation numbers to highlight in green (current citation when citation bar is showing — accept-button green)
  ...
  onAskFollowUpFromCallout?: (messageId: string, citationNumber: string, citationData: any) => void;
  /** Called when user clicks "View in document" on a citation callout preview. Pass messageId and citationNumber so parent can track which citation is open. */
  onViewInDocumentFromCallout?: (citationData: any, messageId: string, citationNumber: string) => void;
  /** Which citation is currently open in the document view (so callout can show "Close document"). */
  ...
  /** Citation bar mode: ordered citation numbers for this message (first-appearance order). When set, only one callout is shown (message-level). */
  orderedCitationNumbersForMessage?: string[];
  /** When false, citation bar is closed (e.g. after Accept on last citation); callouts use persisted acceptedCitationIndices and accepted ones stay hidden. */
  isCitationBarActive?: boolean;
  /** Current citation index (0-based) for the citation bar. */
  currentCitationIndex?: number;
  /** Indices of citations already accepted (hidden). */
  acceptedCitationIndices?: Set<number>;
  /** When true, hide the current callout and show "Review Next Citation" in the bar. */
  showReviewNextOnly?: boolean;
  /** When false, do not show any in-response citation callouts (previous response when there is a new one). */
  showInResponseCitationCallouts?: boolean;
  /** When false, do not show document preview bar or Accept/View bar below citations (citation numbers in text remain). */
  ...
  // Track which citations have already had the blue swoop so we don't re-animate on every re-render when a citation is clicked
  const [blueAnimatedCitationNumbers, setBlueAnimatedCitationNumbers] = React.useState<Set<string>>(() => new Set());
  // Only animate the first occurrence of each citation number (same number can appear multiple times in the message)
  const blueHighlightFirstOccurrenceRef = React.useRef<Set<string>>(new Set());
  ...
  const calloutRectsRef = React.useRef<{ top: number; bottom: number; left: number; right: number }[]>([]);
  ...
  // Ref for isStreaming so markdownComponents useMemo doesn't depend on it (avoids remount when streaming ends)
  ...
  // Show excerpt text (the "writing for which piece it links to") only once per citation; subsequent uses show only the callout card.
  const excerptShownForCitationsRef = React.useRef(new Set<string>());
  const prevMessageIdForExcerptRef = React.useRef<string | undefined>(undefined);
  if (prevMessageIdForExcerptRef.current !== messageId) {
    excerptShownForCitationsRef.current.clear();
  ...
  // Also handle mount/remount with isStreaming already false (e.g. parent re-render replaced list) so feedback bar still shows.
  const prevIsStreamingRef = React.useRef(isStreaming);
  React.useEffect(() => {
    const wasStreaming = prevIsStreamingRef.current;
    prevIsStreamingRef.current = isStreaming;
    const transitionedToDone = wasStreaming && !isStreaming;
    const alreadyDoneOnMount = !wasStreaming && !isStreaming && !!text;
    if (!transitionedToDone && !alreadyDoneOnMount) return;
    if (skipRevealAnimation) return;
    if (alreadyDoneOnMount) {
      if (messageId) onRevealCompleteRef.current?.(messageId);
      return;
    }
    if (revealRafRef.current) {
    ...
    setShowOverlay(false);
    setLines([]);
    // Perplexity-style: no line overlay. If there's a first callout, defer onRevealComplete
    // until the post-unveil text reveal finishes. Otherwise call it now.
    if (usePerplexityStyle) {
      if (firstCalloutBlockIndexRef.current !== null) {
        // There is a citation callout -- onRevealComplete will be called when firstCalloutUnveiled becomes true
      } else {
        if (messageId) onRevealCompleteRef.current?.(messageId);
      }
      return;
  ...
  const measureLines = React.useCallback(() => {
    ...
    // Rects for citation callouts (container coords) so overlay can skip them and show callouts instantly
    const calloutRects: { top: number; bottom: number; left: number; right: number }[] = [];
    container.querySelectorAll('[data-citation-callout]').forEach((el) => {
    ...
    calloutRectsRef.current = calloutRects;
    ...
    if (streamingRef.current) {
      ...
      if (newCount === currentTarget) {
        ...
        if (wrapperRef.current && newCount > 0) {
          ...
          let revealedBottom = rect.top + rect.height;
          container.querySelectorAll('[data-citation-callout]').forEach((el) => {
            const r = (el as HTMLElement).getBoundingClientRect();
            const topInContainer = r.top - containerRect.top;
            const bottomInContainer = r.bottom - containerRect.top;
            if (topInContainer < revealedBottom) revealedBottom = Math.max(revealedBottom, bottomInContainer);
          });
  ...
  const startRevealLoop = React.useCallback(() => {
    ...
    const MAX_STEP_FIRST_LINES = 0.008;
    ...
    const tick = () => {
      ...
      if (rects.length > 0 && overlayCount > 0) {
        ...
        let revealedBottom = rect.top + rect.height;
        // As soon as the reveal reaches the top of a citation callout, include the whole callout so it appears at once instead of line-by-line
        const container = textContainerRef.current;
        if (container) {
          const containerRect = container.getBoundingClientRect();
          container.querySelectorAll('[data-citation-callout]').forEach((el) => {
  ...
  const renderTextSegment = (text: string): React.ReactNode[] => {
    ...
    return parts.length > 0 ? parts : [<React.Fragment key="text-segment">{text}</React.Fragment>];
  };
  ...
  if (blueCitationNumbers) {
    Array.from(blueHighlightFirstOccurrenceRef.current).forEach((num) => {
      if (!blueCitationNumbers.has(num)) blueHighlightFirstOccurrenceRef.current.delete(num);
    });
  }
        ...
        if (n >= 0 && n < mainSegments.length) {
          nodesToAdd.push(
            skipHighlight ? (
              ...
              <MainAnswerHighlight key={`${keyPrefix}-main-${n}`} isStreaming={isStreamingRef.current} runSwoop={runBlueSwoop}>
                {mainSegments[n]}
              </MainAnswerHighlight>
            )
          );
  ...
  // run before a citation (including preceding paragraph text) gets the highlight, not just the inline phrase.
  const explodeInlineByCitation = (child: React.ReactElement, tag: 'strong' | 'em'): (string | React.ReactElement)[] => {
    ...
    React.Children.forEach(childChildren, (c) => {
      if (typeof c === 'string') {
        c.split(citationPlaceholderRe).forEach((part) => {
          pushPart(part, `inline-cit-${partIndex++}`);
        });
      } else if (React.isValidElement(c)) {
        const grandChildren = (c.props as any)?.children;
  ...
  const flattenSegments = (nodes: React.ReactNode): (string | React.ReactElement | symbol)[] => {
    ...
    React.Children.forEach(nodes, (child) => {
      if (typeof child === 'string') {
        ...
        const isIntrinsicStrongOrEm = tag === 'strong' || tag === 'em';
        const isCustomComponent = typeof tag === 'function';
    ...
    const flushPending = (num: string | null, citKey: string, citationNode: React.ReactNode | null): { consumed: boolean; citationPushedInFlush?: boolean } => {
      ...
      if (inHighlight && pending.length > 0) {
        ...
        if (typeof pendingToUse[0] === 'string') {
          ...
          leadingSpace = m ? m[0] : '';
          if (leadingSpace) pendingToUse[0] = first.slice(leadingSpace.length);
        }
      }
      ...
      if (typeof child === 'string') {
        ...
        parts.forEach((part, idx) => {
          ...
          if (part.startsWith('%%CITATION_')) {
            ...
            const nextPart = parts[idx + 1];
            const nextCitNum = nextPart ? citationNumFromPlaceholder(nextPart) : null;
            if (nextCitNum == null || !(rejectedCitationNumbers?.has(nextCitNum) ?? false)) {
              ...
              const wrapGreen = nextCitNum != null && (greenCitationNumbers?.has(nextCitNum) ?? false);
              const wrapBlue = nextCitNum != null && (blueCitationNumbers?.has(nextCitNum) ?? false);
      ...
      React.Children.forEach(node, (child) => {
        if (typeof child === 'string') {
          ...
          for (const p of parts) {
            if (typeof p === 'string' && p.startsWith('%%CITATION_')) {
              const n = citationNumFromPlaceholder(p);
              if (n) nums.push(n);
            }
          }
        } else if (React.isValidElement(child)) {
  ...
  // True if this block would render no visible content after removing rejected citations (so we can hide empty list items).
  const blockWouldBeEmptyAfterRejected = (nodes: React.ReactNode): boolean => {
    const segments = flattenSegments(nodes);
    for (let i = 0; i < segments.length; i++) {
      const seg = segments[i];
      if (typeof seg === 'string') {
        if (seg.startsWith('%%CITATION_')) {
          const num = citationNumFromPlaceholder(seg);
          if (num == null || !(rejectedCitationNumbers?.has(num) ?? false)) return false;
        } else {
          const nextSeg = segments[i + 1];
          const nextNum = typeof nextSeg === 'string' && nextSeg?.startsWith('%%CITATION_') ? citationNumFromPlaceholder(nextSeg) : null;
          if (nextNum == null || !(rejectedCitationNumbers?.has(nextNum) ?? false)) return false;
        }
      } else return false;
    }
    return true;
  ...
  // Plain text of a block for comparison (strips citation placeholders and [id: N](...) refs, normalizes whitespace). Used to hide duplicate excerpt lines.
  const getBlockPlainTextForComparison = (nodes: React.ReactNode): string => {
    const segments = flattenSegments(nodes);
    const parts: string[] = [];
    const refPattern = /\[id:\s*\d+\]\([^)]*\)/g; // full ref e.g. [id: 12](block_cite_id_34)
    const incompleteRefPattern = /\[id:\s*\d+\]\([^)]*$/g; // incomplete ref e.g. [id: 12](blockciteid with no closing paren
    for (const seg of segments) {
      if (typeof seg === 'string' && !seg.startsWith('%%CITATION_')) {
        parts.push(seg.replace(refPattern, '').replace(incompleteRefPattern, ''));
      }
    }
    return parts.join('').replace(/\s+/g, ' ').trim();
  };

  // Normalize for excerpt comparison: collapse spaces and optional commas so "to the bank secrecy act , ," can match excerpt tail.
  ...
  const currentCitationNum = citationBarMode && orderedCitationNumbersForMessage && currentCitationIndex >= 0 && currentCitationIndex < orderedCitationNumbersForMessage.length
    ? orderedCitationNumbersForMessage[currentCitationIndex]
    : null;
  // Show callout for current citation whenever it's selected (e.g. via arrow nav), even if previously accepted, so the preview bar appears when going back to accepted citations
  ...
  const markdownComponents = React.useMemo(() => {
    ...
    const showCalloutForNum = (num: string) => {
      if (rejectedCitationNumbers?.has(num)) return false;
    ...
    const hasCalloutsBelow = (citationNumbers: string[]) => {
      if (!showCitationPreviewBar || !showInResponseCitationCallouts) return false;
      if (citationNumbers.length === 0) return false;
      if (citationBarMode && showReviewNextOnly) return false;
    ...
    const renderCalloutOrFirstUnveil = (num: string, keyPrefix: string, i: number) =>
      num === firstCitationNum ? renderFirstCalloutWithUnveil() : renderCallout(num, keyPrefix, i);
    return {
    p: ({ children }: { children?: React.ReactNode }) => {
      ...
      const naturalFirstCitationLayout = containsFirst && firstCitationNum && firstSegs.length > 0;
      // Lock in first-citation layout once we've rendered the wrapper so streaming doesn't unmount/remount it (avoids animation spam)
      const useFirstCitationLayout = naturalFirstCitationLayout || (firstCalloutBlockIndexRef.current !== null && containsFirst && firstCitationNum != null);
      const firstPartContent = useFirstCitationLayout ? (processFlattenedWithCitations(firstSegs, 'p-first') ?? processChildrenWithCitationsFlattened(children ?? null, 'p-first')) : null;
      ...
      const firstPartContentToRender = firstPartContent != null && (Array.isArray(firstPartContent) ? firstPartContent.length > 0 : true) ? firstPartContent : (useFirstCitationLayout ? processChildrenWithCitationsFlattened(children ?? null, 'p-first') : null);
          ...
          <>
            ...
            {restPartContent != null && restPartContent.length > 0 && (
              <div style={{ ...getPostCalloutStyle(), marginTop: 4 }}>
                ...
                {citationBarMode && renderSingleCalloutIfHere(restCitationNums, 'p-rest')}
              </div>
            )}
          </>
        );
      ...
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const postCalloutStyle = isPostCalloutBlock ? getPostCalloutStyle() : undefined;
      const pInner = processChildrenWithCitationsFlattened(children ?? null, 'p');
      ...
      const content = (
        <>
          {(!isOnlyCitationExcerpt || shouldShowExcerptThisTime) && (
            ...
            }}><span style={{ display: 'block', lineHeight: '1.7' }}>{showBar && <span aria-hidden style={citationLineBarInlineStyle} />}{pContent}</span></p>
          )}
      ...
      const content = (
        <>
          <h1 style={{
            ...
            wordBreak: 'break-word',
            ...(showBar ? citationLineBarBlockStyle : {}),
          }}>{showBar && <span aria-hidden style={citationLineBarInlineStyle} />}{isPostCalloutBlock ? wrapTwoWordChunksInMotion(processChildrenWithCitationsFlattened(children ?? null, 'h1'), `h1-puv-${blockIndexRef.current}`) : processChildrenWithCitationsFlattened(children ?? null, 'h1')}</h1>
          {showCitationPreviewBar && showInResponseCitationCallouts && !citationBarMode && citationNumbers.filter(showCalloutForNum).map((num, i) => renderCalloutOrFirstUnveil(num, 'callout-h1', i))}
      ...
      const content = (
        <>
          <h2 style={{
            ...
            wordBreak: 'break-word',
            ...(showBar ? citationLineBarBlockStyle : {}),
          }}>{showBar && <span aria-hidden style={citationLineBarInlineStyle} />}{isPostCalloutBlock ? wrapTwoWordChunksInMotion(processChildrenWithCitationsFlattened(children ?? null, 'h2'), `h2-puv-${blockIndexRef.current}`) : processChildrenWithCitationsFlattened(children ?? null, 'h2')}</h2>
          {showCitationPreviewBar && showInResponseCitationCallouts && !citationBarMode && citationNumbers.filter(showCalloutForNum).map((num, i) => renderCalloutOrFirstUnveil(num, 'callout-h2', i))}
      ...
      const content = (
        <>
          <h3 style={{
            ...
            wordBreak: 'break-word',
            ...(showBar ? citationLineBarBlockStyle : {}),
          }}>{showBar && <span aria-hidden style={citationLineBarInlineStyle} />}{isPostCalloutBlock ? wrapTwoWordChunksInMotion(processChildrenWithCitationsFlattened(children ?? null, 'h3'), `h3-puv-${blockIndexRef.current}`) : processChildrenWithCitationsFlattened(children ?? null, 'h3')}</h3>
          {showCitationPreviewBar && showInResponseCitationCallouts && !citationBarMode && citationNumbers.filter(showCalloutForNum).map((num, i) => renderCalloutOrFirstUnveil(num, 'callout-h3', i))}
    ...
    li: ({ children }: { children?: React.ReactNode }) => {
      ...
      const showBar = hasCalloutsBelow(citationNumbers);
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const liPostCalloutStyle = isPostCalloutBlock ? getPostCalloutStyle() : undefined;
      return (
        <li style={{
          ...
          {(() => {
            const liInner = processChildrenWithCitationsFlattened(children ?? null, 'li');
            const liContent = isPostCalloutBlock ? wrapTwoWordChunksInMotion(liInner, `li-puv-${blockIndexRef.current}`) : liInner;
            return showBar ? (
              <div style={{ position: 'relative' }}>
                <span aria-hidden style={citationLineBarLiBarStyle} />
                {liContent}
              </div>
            ) : liContent;
          })()}
          {showCitationPreviewBar && showInResponseCitationCallouts && !citationBarMode && citationNumbers.filter(showCalloutForNum).map((num, i) => renderCalloutOrFirstUnveil(num, 'callout-li', i))}
    ...
    strong: ({ children }: { children?: React.ReactNode }) => {
      ...
      return <MainAnswerHighlight isStreaming={isStreamingRef.current} runSwoop={runBlueSwoop}>{boldContent}</MainAnswerHighlight>;
    },
    em: ({ children }: { children?: React.ReactNode }) => {
      return <em style={{ 
        ...
        overflowWrap: 'break-word',
        wordBreak: 'break-word'
      }}><>{processChildrenWithCitationsFlattened(children ?? null, 'em')}</></em>;
    },
      ...
      const content = (
        <>
          <blockquote style={{
            ...
            overflowWrap: 'break-word',
            wordBreak: 'break-word'
          }}>{showBar && <span aria-hidden style={citationLineBarInlineStyle} />}{isPostCalloutBlock ? wrapTwoWordChunksInMotion(processChildrenWithCitationsFlattened(children ?? null, 'blockquote'), `bq-puv-${blockIndexRef.current}`) : processChildrenWithCitationsFlattened(children ?? null, 'blockquote')}</blockquote>
          {showCitationPreviewBar && showInResponseCitationCallouts && !citationBarMode && citationNumbers.filter(showCalloutForNum).map((num, i) => renderCalloutOrFirstUnveil(num, 'callout-blockquote', i))}
  ...
  const perplexityMarkdownComponents = React.useMemo(() => {
    ...
    const renderCalloutOrFirstUnveilP = (num: string, keyPrefix: string, i: number) =>
      num === firstCitationNumP ? renderFirstCalloutWithUnveilP() : renderCalloutP(num, keyPrefix, i);
    return {
    ...markdownComponents,
    p: ({ children }: { children?: React.ReactNode }) => {
      blockIndexRef.current++;
      ...
      const naturalFirstCitationLayoutP = containsFirstP && firstCitationNumP && firstSegsP.length > 0;
      const useFirstCitationLayoutP = naturalFirstCitationLayoutP || (firstCalloutBlockIndexRef.current !== null && containsFirstP && firstCitationNumP != null);
      ...
      const firstPartContentPToRender = firstPartContentP != null && (Array.isArray(firstPartContentP) ? firstPartContentP.length > 0 : true) ? firstPartContentP : (useFirstCitationLayoutP ? processChildrenWithCitationsFlattened(children ?? null, 'p-first') : null);
      if (useFirstCitationLayoutP && firstPartContentPToRender != null) {
        const innerFirst = wrapTwoWordChunksInMotion(firstPartContentPToRender, 'p-first');
          ...
          <>
            ...
            {innerRest != null && (
              <div style={{ ...getPostCalloutStyleP(), marginTop: 4 }}>
                ...
                {citationBarMode && renderSingleCalloutIfHere(restCitationNumsP, 'p-rest')}
              </div>
            )}
          </>
        );
      ...
      const showBar = hasCalloutsBelow(citationNumbers);
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const postCalloutStyle = isPostCalloutBlock ? getPostCalloutStyleP() : undefined;
      const pProcessed = processChildrenWithCitationsFlattened(children ?? null, 'p');
      const inner = isPostCalloutBlock
        ? wrapTwoWordChunksInMotion(pProcessed, `p-puv-${blockIndexRef.current}`)
        : wrapTwoWordChunksInMotion(pProcessed, 'p');
      const content = (
        <>
          {(!isOnlyCitationExcerpt || shouldShowExcerptThisTime) && (
            <p style={{
          ...
          {showCitationPreviewBar && showInResponseCitationCallouts && !citationBarMode && citationNumbers.filter(showCalloutForNum).map((num, i) => renderCalloutOrFirstUnveilP(num, 'callout-p', i))}
          {citationBarMode && renderSingleCalloutIfHere(citationNumbers, 'p')}
        </>
      );
      return postCalloutStyle ? <div style={postCalloutStyle}>{content}</div> : content;
    ...
    h1: ({ children }: { children?: React.ReactNode }) => {
      ...
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const postCalloutStyle = isPostCalloutBlock ? getPostCalloutStyleP() : undefined;
      const h1Processed = processChildrenWithCitationsFlattened(children ?? null, 'h1');
      const h1Content = isPostCalloutBlock ? wrapTwoWordChunksInMotion(h1Processed, `h1-puv-${blockIndexRef.current}`) : wrapTwoWordChunksInMotion(h1Processed, 'h1');
      const content = (
      <>
        <h1 style={{
          fontSize: '33px',
    ...
    h2: ({ children }: { children?: React.ReactNode }) => {
      ...
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const postCalloutStyle = isPostCalloutBlock ? getPostCalloutStyleP() : undefined;
      const h2Processed = processChildrenWithCitationsFlattened(children ?? null, 'h2');
      const h2Content = isPostCalloutBlock ? wrapTwoWordChunksInMotion(h2Processed, `h2-puv-${blockIndexRef.current}`) : wrapTwoWordChunksInMotion(h2Processed, 'h2');
      const content = (
      <>
        <h2 style={{
          fontSize: '23px',
    ...
    h3: ({ children }: { children?: React.ReactNode }) => {
      ...
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const postCalloutStyle = isPostCalloutBlock ? getPostCalloutStyleP() : undefined;
      const h3Processed = processChildrenWithCitationsFlattened(children ?? null, 'h3');
      const h3Content = isPostCalloutBlock ? wrapTwoWordChunksInMotion(h3Processed, `h3-puv-${blockIndexRef.current}`) : wrapTwoWordChunksInMotion(h3Processed, 'h3');
      const content = (
      <>
        <h3 style={{
          fontSize: '21px',
    ...
    li: ({ children }: { children?: React.ReactNode }) => {
      ...
      const showBar = hasCalloutsBelow(citationNumbers);
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const liPostCalloutStyle = isPostCalloutBlock ? getPostCalloutStyleP() : undefined;
      const liProcessed = processChildrenWithCitationsFlattened(children ?? null, 'li');
      const liContent = isPostCalloutBlock ? wrapTwoWordChunksInMotion(liProcessed, `li-puv-${blockIndexRef.current}`) : wrapTwoWordChunksInMotion(liProcessed, 'li');
      return (
        <li style={{
          margin: '4.4px 0 4.4px 0',
          lineHeight: '1.7',
    ...
    blockquote: ({ children }: { children?: React.ReactNode }) => {
      ...
      const isPostCalloutBlock = firstCalloutBlockIndexRef.current !== null && blockIndexRef.current > firstCalloutBlockIndexRef.current;
      const postCalloutStyle = isPostCalloutBlock ? getPostCalloutStyleP() : undefined;
      const bqProcessed = processChildrenWithCitationsFlattened(children ?? null, 'blockquote');
      const bqContent = isPostCalloutBlock ? wrapTwoWordChunksInMotion(bqProcessed, `bq-puv-${blockIndexRef.current}`) : wrapTwoWordChunksInMotion(bqProcessed, 'blockquote');
      const content = (
        <>
          <blockquote style={{
            ...(showBar ? citationLineBarBlockStyle : { borderLeft: '3px solid #d1d5db', paddingLeft: '13.1px' }),
        ...
        {!skipRevealAnimation && !usePerplexityStyle && showOverlay && lines.length > 0 && (
          <div style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, pointerEvents: 'none' }}>
            {lines.map((line, i) => {
              ...
              const overlapsCallout = calloutRectsRef.current.some(
                (c) => line.top < c.bottom && lineBottom > c.top
              );
              if (overlapsCallout) return null;
              return (
                <div
                  key={i}
                  style={{
                    ...
                    background: 'linear-gradient(to right, transparent 0%, rgba(255,255,255,0.08) 15%, rgba(255,255,255,0.4) 35%, rgba(255,255,255,0.85) 55%, #FFFFFF 75%, #FFFFFF 100%)',
                    pointerEvents: 'none',
                    filter: 'blur(0.5px)',
...
function setsEqualNumber(a: Set<number> | undefined, b: Set<number> | undefined): boolean {
  if (a === b) return true;
  if (!a || !b) return !a && !b;
  if (a.size !== b.size) return false;
  for (const k of a) if (!b.has(k)) return false;
...
function streamingResponseTextAreEqual(
  ...
  const prevViewed = prev.citationViewedInDocument;
  const nextViewed = next.citationViewedInDocument;
  const viewedEqual = (prevViewed == null && nextViewed == null) ||
    (prevViewed != null && nextViewed != null && prevViewed.messageId === nextViewed.messageId && prevViewed.citationNumber === nextViewed.citationNumber);
  const orderedSame = (prev.orderedCitationNumbersForMessage === next.orderedCitationNumbersForMessage) ||
  ...
  return (
    ...
    setsEqual(prev.greenCitationNumbers, next.greenCitationNumbers) &&
    setsEqual(prev.rejectedCitationNumbers, next.rejectedCitationNumbers) &&
...
function getOrderedCitationNumbersFromMessageText(text: string): string[] {
  ...
  // Collect (index, num) so we can merge by first appearance
  const hits: { index: number; num: string }[] = [];
  // Placeholders (if text was already processed)
  const placeholderRe = /%%CITATION_(?:SUPERSCRIPT|BRACKET|PENDING)_(\d+)%%/g;
  let m: RegExpExecArray | null;
  while ((m = placeholderRe.exec(text)) !== null) hits.push({ index: m.index, num: m[1]! });
  ...
  const superscriptRe = /[¹²³⁴⁵⁶⁷⁸⁹]+(?:\d+)?/g;
  while ((m = superscriptRe.exec(text)) !== null) {
    let numStr = '';
    for (const char of m[0]) numStr += superscriptMap[char] ?? (/\d/.test(char) ? char : '');
    if (numStr) hits.push({ index: m.index, num: numStr });
  }
  hits.sort((a, b) => a.index - b.index);
  for (const { num } of hits) add(num);
  return order;
}
...
function getCitedRunFromMessageText(messageText: string, citationNumber: string): string {
  ...
  const superscriptRe = /[¹²³⁴⁵⁶⁷⁸⁹]+(?:\d+)?/g;
  while ((m = superscriptRe.exec(messageText)) !== null) {
    let numStr = '';
    for (const char of m[0]) numStr += superscriptMap[char] ?? (/\d/.test(char) ? char : '');
    if (numStr) hits.push({ index: m.index, end: m.index + m[0].length, num: numStr });
  }
  hits.sort((a, b) => a.index - b.index);
  let prevEnd = 0;
  let lastNonEmptyRun = '';
        ...
        onClick={(e) => {
          ...
          if (citedBlock) {
            ...
            while (el) {
              ...
              if (el.classList?.contains('citation-link-btn')) {
                const r = el.getBoundingClientRect();
                const left = Math.min(blockRect.left, r.left);
                const top = Math.min(blockRect.top, r.top);
                const right = Math.max(blockRect.right, r.right);
                const bottom = Math.max(blockRect.bottom, r.bottom);
                anchorRect = new DOMRect(left, top, right - left, bottom - top);
...
function renderExcerptWithFormatting(text: string): React.ReactNode {
  ...
  for (let i = 0; i < boldSplit.length; i++) {
    if (i % 2 === 1) {
      ...
      for (let j = 0; j < italicSplit.length; j++) {
        if (j % 2 === 1) {
          parts.push(<em key={`ee-${keyIdx++}`} style={{ fontStyle: 'italic' }}>{italicSplit[j]}</em>);
        } else if (italicSplit[j]) {
          parts.push(<React.Fragment key={`et-${keyIdx++}`}>{italicSplit[j]}</React.Fragment>);
        }
      }
    }
  }
...
/** Wraps a message block and reports when it is in view; used so citation bar can show when scrolling back to accepted messages. */
const CitationMessageVisibility: React.FC<{
  messageId: string;
  onVisibilityChange: (messageId: string, inView: boolean) => void;
  observe: boolean;
  children: React.ReactNode;
}> = ({ messageId, onVisibilityChange, observe, children }) => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          onVisibilityChange(messageId, entry.isIntersecting);
        });
      },
  ...
  }, [messageId, onVisibilityChange, observe]);
...
const CitationCalloutUnveilWrapper: React.FC<{
  ...
  onUnveilCompleteRef.current = onUnveilComplete;
  ...
  React.useLayoutEffect(() => {
    ...
    const fallback = requestAnimationFrame(() => {
      const el2 = measureRef.current;
      if (el2) setMeasuredHeight(el2.scrollHeight);
    });
  ...
  // Phase 1: measure in an in-flow hidden div so layout has full space and we get correct scrollHeight
  ...
  // Phase 4: animation finished — render at natural size so container is never cut off
  if (animationDone) {
    return <div style={{ width: '100%', minWidth: 0 }}>{children}</div>;
  }

  // Phase 2: delay — reserve layout space but keep content clipped (invisible)
  if (!delayDone) {
    return (
      <div style={{ height: measuredHeight, overflow: 'hidden', minWidth: 0 }}>
        <div style={{ width: '100%', minWidth: 0, clipPath: 'inset(0 0 100% 0)' }}>
          {frozenChildrenRef.current}
        </div>
      </div>
    );
  ...
  // Phase 3: fixed height + clip-path reveal (stable layout during animation)
  return (
    <div style={{ height: measuredHeight, overflow: 'hidden', minWidth: 0 }}>
      <motion.div
...
const CitationCallout: React.FC<{
  ...
  onCloseCallout?: () => void;
  /** Called when user clicks the × next to Accept; closes document preview cards and bar for this message (parent shows "Open" button). */
  onClosePreviewBar?: () => void;
  /** When true, hide Accept and View document (they live in the chat bar); show Ask Question on hover only. */
  ...
  const displayText = (messageCitedExcerptProp ?? '').trim() || fallbackDisplay;

  const docId = citation?.doc_id ?? (citation as { document_id?: string })?.document_id;
  const pageNum = citation?.page ?? citation?.bbox?.page ?? (citation as { page_number?: number })?.page_number ?? 1;
  ...
  const [cachedPageImage, setCachedPageImage] = React.useState<CachedPageImage | null>(() => {
    if (!docId || !hasBbox || isWordDoc) return null;
    const key = `hover-${docId}-${pageNum}`;
    const c = hoverPreviewCache.get(key);
  ...
  /** When true, show only icons in action buttons (Ask Question, View document, etc.) to fit narrow chat bar. */
  ...
  React.useEffect(() => {
    ...
    if (cached) {
      setCachedPageImage({ pageImage: cached.pageImage, imageWidth: cached.imageWidth, imageHeight: cached.imageHeight });
      return;
    }
    let cancelled = false;
  ...
  }, [cachedPageImage, hasBbox, bbox, previewSize.width, previewSize.height]);

  const showPreviewImage = canShowPreview && cachedPageImage;
  const docTypeLabel = citation?.classification_type != null
    ? String(citation.classification_type).replace(/_/g, ' ')
    : 'PDF Document';
  ...
  const citationCalloutEntrance = { initial: { opacity: 1, y: 0 } as const, animate: { opacity: 1, y: 0 } as const, transition: { duration: 0 } as const };
  React.useEffect(() => {
          ...
          {canShowPreview ? (
            <div style={{ position: 'relative', width: '100%', height: 180, minHeight: 180, flexShrink: 0, boxSizing: 'border-box' }}>
              ...
              >
                ...
                >
                  {/* Ask about this... chat bar (send button inside input) */}
                  {showAskQuestion && (
...
const citationRunLengthSuperscriptMap: Record<string, string> = {
  '¹': '1', '²': '2', '³': '3', '⁴': '4', '⁵': '5',
  '⁶': '6', '⁷': '7', '⁸': '8', '⁹': '9'
};
const citationRunLengthRe = /\[(\d+)\]|[¹²³⁴⁵⁶⁷⁸⁹]+(?:\d+)?/g;

/** Returns a map of citation number -> how many citations are in a row (adjacent in text). Only whitespace/comma between counts as "in a row". */
function getCitationRunLengthMap(text: string): Map<string, number> {
  const map = new globalThis.Map<string, number>();
  if (!text) return map;
  const matches: { num: string; start: number; end: number }[] = [];
  let m: RegExpExecArray | null;
  while ((m = citationRunLengthRe.exec(text)) !== null) {
    const raw = m[0];
    const num = raw.startsWith('[') ? (m[1] ?? '') : Array.from(raw).map((c) => citationRunLengthSuperscriptMap[c] ?? (/\d/.test(c) ? c : '')).join('') || raw;
    if (num) matches.push({ num, start: m.index, end: m.index + m[0].length });
  }
  if (matches.length === 0) return map;
  let run: string[] = [matches[0].num];
  for (let i = 1; i < matches.length; i++) {
    const gap = text.slice(matches[i - 1].end, matches[i].start);
    const onlyWhitespaceOrComma = /^[\s,]*$/.test(gap);
    if (onlyWhitespaceOrComma) {
      run.push(matches[i].num);
    } else {
      const len = run.length;
      run.forEach((n) => map.set(n, len));
      run = [matches[i].num];
    }
  }
  const len = run.length;
...
/** Returns how many citations are in a row (adjacent in text) for the given citation number. Only whitespace/comma between counts as "in a row". */
function getCitationAdjacentRunLength(text: string, citationNumber: string): number {
  if (!text || citationNumber == null) return 1;
  return getCitationRunLengthMap(text).get(citationNumber) ?? 1;
}
...
interface SideChatPanelProps {
  ...
  /** Timestamp from MainContent when a query was submitted from dashboard/map; triggers brief chat bar border glow. */
...
export const SideChatPanel = React.forwardRef<SideChatPanelRef, SideChatPanelProps>(({
  ...
  const [citationReviewMessageId, setCitationReviewMessageId] = React.useState<string | null>(null);
  const [citationReviewCurrentIndex, setCitationReviewCurrentIndex] = React.useState(0);
  const [citationReviewAcceptedIndices, setCitationReviewAcceptedIndices] = React.useState<Set<number>>(() => new Set());
  ...
  const [citationReviewJustRejected, setCitationReviewJustRejected] = React.useState(false);
  /** Message IDs for which the user has finished review (e.g. accepted last citation) — bar is hidden for these. */
  const [citationReviewCompletedForMessageIds, setCitationReviewCompletedForMessageIds] = React.useState<globalThis.Set<string>>(() => new globalThis.Set());
  const citationReviewCompletedForMessageIdsRef = React.useRef(citationReviewCompletedForMessageIds);
  React.useEffect(() => {
    citationReviewCompletedForMessageIdsRef.current = citationReviewCompletedForMessageIds;
  }, [citationReviewCompletedForMessageIds]);
  /** Message IDs (with citations) currently in view — used to show citation bar when scrolling back to accepted messages; only one bar open at a time. */
  const [citationMessagesInView, setCitationMessagesInView] = React.useState<globalThis.Set<string>>(() => new globalThis.Set());
  /** Per-message last citation index when user closed the document or bar — restored when they click "Open" again. */
  const [lastClosedCitationIndexByMessageId, setLastClosedCitationIndexByMessageId] = React.useState<Record<string, number>>(() => ({}));
  const handleCitationVisibilityChange = React.useCallback((messageId: string, inView: boolean) => {
    setCitationMessagesInView((prev) => {
      const next = new globalThis.Set(prev);
      if (inView) next.add(messageId);
  ...
  const saveCitationForDocx = React.useCallback(async (messageId: string, citationNumber: string, citationData: CitationData & { document_id?: string; bbox?: { left: number; top: number; width: number; height: number } }): Promise<boolean> => {
    ...
    const docId = citationData.document_id ?? (citationData as { doc_id?: string }).doc_id;
    const pageNum = citationData.page ?? citationData.bbox?.page ?? (citationData as { page_number?: number }).page_number ?? 1;
    const bbox = citationData.bbox;
    ...
    try {
      ...
      citationExport.setCitationExportData((prev) => ({
        ...
        [messageId]: {
          ...(prev[messageId] ?? {}),
          [citationNumber]: { type: 'copy' as const, imageDataUrl: cropped },
        },
      }));
      return true;
    } catch {
  ...
  const handleDownloadResponseAsDocxForMessage = React.useCallback(async (text: string, messageId: string) => {
    try {
      const md = await buildDocxMarkdownWithCitationImages(text, messageId, {});
  ...
  // When parent triggers chat bar glow (query submitted from dashboard/map), show brief border glow
  React.useEffect(() => {
    if (chatBarGlowTrigger == null || chatBarGlowTrigger === 0) return;
    startChatBarGlow();
  }, [chatBarGlowTrigger, startChatBarGlow]);
  ...
  // State for citation preview bar toggle (document preview + Accept/View bar) - persisted to localStorage
  const [showCitationPreviewBar, setShowCitationPreviewBar] = React.useState<boolean>(() => {
    try {
      const saved = localStorage.getItem('showCitationPreviewBar');
      if (saved === null) return true; // Default ON
      const parsed = JSON.parse(saved);
      return parsed === true;
    } catch {
  ...
  React.useLayoutEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      // QuickStartBar should match chat bar width (680px) for alignment
      quickStartWrapper.style.maxWidth = '720px'; // Match content wrapper maxWidth
  ...
  // Latest assistant message by position so citation bar/callouts never apply to a previous response once a new one exists
  const latestAssistantMessageKey = React.useMemo(() => {
    const list = Array.isArray(chatMessages) ? chatMessages : [];
    const last = [...list].reverse().find((m: { type?: string }) => m.type !== 'query');
    return last ? ((last as { id?: string }).id ?? `msg-${list.indexOf(last)}`) : null;
  }, [chatMessages]);
  ...
  React.useEffect(() => {
    ...
    const list = Array.isArray(chatMessages) ? chatMessages : [];
    const current = citationReviewMessageIdRef.current;
    if (current != null && citationMessagesInView.has(current)) {
      return;
    }
    const last = [...list].reverse().find((m: { type?: string }) => m.type !== 'query');
    const latestKey = last ? ((last as { id?: string }).id ?? `msg-${list.indexOf(last)}`) : null;
    let bestId: string | null = null;
    list.forEach((m) => {
      const id = (m as { id?: string }).id ?? `msg-${list.indexOf(m)}`;
      if (citationMessagesInView.has(id) && (m as { type?: string }).type !== 'query' && (m as { text?: string }).text) {
        bestId = id;
      }
    });
    if (bestId != null) {
      if (citationReviewCompletedForMessageIdsRef.current.has(bestId) && bestId === latestKey) {
        setCitationReviewMessageId(null);
        citationReviewMessageIdRef.current = null;
        return;
      }
      citationReviewMessageIdRef.current = bestId;
      setCitationReviewMessageId(bestId);
    } else {
      citationReviewMessageIdRef.current = null;
      setCitationReviewMessageId(null);
    }
  }, [citationMessagesInView, chatMessages]);
  ...
  const closeExpandedCardView = React.useCallback(() => {
    const chatId = currentChatIdRef.current;
    if (chatId) {
      const viewed = getChatState(chatId)?.documentPreview?.viewedCitation ?? null;
      if (viewed) {
        ...
        (async () => {
          try {
            ...
            const processBlockQueue = () => {
              ...
              const processNext = () => {
                ...
                if (!stillActive) {
                  ...
                  // Flush any remaining blocks so finalize has full content and queue drains
            ...
            await backendApi.queryDocumentsStreamFetch(
              ...
              (data: any) => {
                setTransientThinkingMessageId(null);
                if (usePerplexityStyleRef.current) {
                ...
                const checkQueue = setInterval(() => {
                  ...
                  if (!isProcessingQueue && blockQueue.length === 0 && !tokenBuffer.trim() && !pendingBuffer.trim()) {
                    ...
                    if (usePerplexityStyleRef.current) {
                      perplexityFinalizeRef.current = () => finalizeText(true);
                    } else {
                      finalizeText(true);
                    }
  ...
  // No continuous scroll during streaming - user can scroll up freely; scrollToBottom() is still called from onTextUpdate when autoScrollEnabledRef is true.
  ...
  React.useEffect(() => {
    ...
    if (documentJustClosed && !isNavigatingTaskRef.current) {
      ...
      // Reset flags after layout has applied (no closing animation - instant snap)
      setTimeout(() => {
        setJustEnteredFullscreen(false);
  ...
  const handleUserCitationClick = React.useCallback((data: CitationDataType, anchorRect?: DOMRect, highlightRect?: DOMRect | null, sourceMessageText?: string, messageId?: string, citationNumber?: string) => {
    ...
    if (messageId != null && latestAssistantMessageKey != null && messageId !== latestAssistantMessageKey) return;
    if (anchorRect != null) {
  ...
  React.useEffect(() => {
    if (isVisible) {
      ...
      // If we have persisted messages and the panel state is empty, restore them (no animation).
      // CRITICAL: Only run when chatMessages.length === 0 so we don't treat "streaming in progress"
      // as a restore. During streaming the ref is kept in sync with current messages and query is
      // cleared, so without this check we would overwrite state and auto-open the doc on every run.
      if (persistedChatMessagesRef.current.length > 0 && (!query || !query.trim()) && chatMessages.length === 0) {
        ...
        // Automatically open document preview if persisted messages have citations (only on restore)
          ...
          await backendApi.queryDocumentsStreamFetch(
            ...
            (data: any) => {
              ...
              console.log('✅ SideChatPanel: LLM streaming complete:', {
                ...
                // For active chats: use ref (updated on every onReasoningStep) so we don't lose "Summarising content" if stream end runs before state commit
                // For inactive chats, use buffered state
                let latestReasoningSteps: ReasoningStep[] = [];
                if (chatIsActive) {
                  const activeMessage = chatMessages.find(msg => msg.id === loadingResponseId);
                  const refSteps = loadingReasoningStepsRef.current;
                  latestReasoningSteps = (refSteps?.length ? refSteps : activeMessage?.reasoningSteps) || [];
                } else if (queryChatId) {
                  // Inactive chat - use buffered state
                  const bufferedState = getBufferedState(queryChatId);
            ...
            (step: { step: string; action_type?: string; message: string; count?: number; details: any }) => {
              ...
              // Transient "Thinking": show in parallel (same slot), don't add as sequential step
              if (step.step === 'thinking') {
                setTransientThinkingMessageId(loadingResponseId);
                return;
              }
              setTransientThinkingMessageId(null);
                ...
                setChatMessages(prev => {
                  const updated = prev.map(msg => {
                    if (msg.id === loadingResponseId) {
                      ...
                      loadingReasoningStepsRef.current = newSteps;
                      return { ...msg, reasoningSteps: newSteps };
                    }
                    return msg;
                  });
                  persistedChatMessagesRef.current = updated;
  ...
  const renderedMessages = useMemo(() => {
    ...
    // Only the latest assistant message (by position) gets citation bar/callout; previous responses never do
    const lastAssistant = [...validMessages].reverse().find(({ message }) => message.type !== 'query');
    ...
    return validMessages.map(({ message, idx }) => {
      ...
      return (
        <CitationMessageVisibility
          key={finalKey}
          messageId={finalKey}
          onVisibilityChange={handleCitationVisibilityChange}
        ...
        <div
            ...
            <div style={{
              ...
              <StreamingResponseTextMemo
                ...
                greenCitationNumbers={(() => {
                  ...
                  const set = new Set<string>();
                  set.add(currentReviewCitationNum);
                  return set;
                })()}
                ...
                onAskFollowUpFromCallout={(msgId, citationNumber, citationData) => {
                  ...
                  const docId = data?.document_id ?? (data as any)?.doc_id;
                  const pageNum = data?.page ?? data?.page_number ?? (data?.bbox as any)?.page ?? 1;
                  const bbox = data?.bbox ?? { left: 0, top: 0, width: 0, height: 0 };
                  if (docId) {
                    preloadCitationBboxSegment({ document_id: docId, page_number: pageNum, bbox });
                  }
                  const raw = (data?.cited_text || data?.block_content || 'this citation').trim().slice(0, 200);
                  const snippet = raw.replace(/\*\*/g, '').replace(/__/g, '');
                  const id = `cite-${docId ?? 'doc'}-${pageNum}-${Date.now()}`;
                  const sourceMessageText = message.text != null ? message.text.slice(-2000) : undefined;
                  segmentInput.insertChipAtCursor(
                    {
                      type: 'chip',
                      ...
                      label: snippet,
                      payload: {
                        citationData: { ...data, block_id: data?.block_id, cited_text: snippet },
                        sourceMessageText,
                        messageId: msgId,
                        citationNumber,
                      },
              ...
              requestAnimationFrame(() => {
                ...
                showBlueCitationHighlight={showBlueCitationHighlight && !citationPreviewClosedForMessageIds.has(finalKey)}
              />
            </div>
          )}
          ...
          {message.text && hasCurrentStreamFinished && (
            ...
            >
              {revealEndedOrRestored && showFeedbackBar && (() => {
                const sources = getSourcesFromReadingStepsOrCitations(message);
                const isSourcesOpen = sourcesDropdownMessageId === finalKey;
                const prevMessage = validMessages[idx - 1]?.message;
                const hasNewFilesFromQuery = prevMessage?.type === 'query' && (prevMessage as any).attachments?.length > 0 &&
                  (prevMessage as any).attachments.some((a: any) => a?.file instanceof File);
                return (
            <div className="feedback-action-bar-enter" style={{ display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap', color: '#6b7280', fontSize: '11px', fontFamily: 'inherit' }}>
              ...
              <Tooltip>
                ...
                style={{
                  ...
                  ...(likedResponseIds.has(finalKey) ? { backgroundColor: 'rgba(34, 197, 94, 0.12)' } : {}),
              ...
              {!likedResponseIds.has(finalKey) && (
                <Tooltip>
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer',
                    ...(dislikedResponseIds.has(finalKey) ? { backgroundColor: '#E5E7EB' } : {}),
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                      <button
                        ...
                        style={{ padding: '2px 8px', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.4)', background: 'transparent', color: 'white', cursor: 'pointer', fontSize: '11px' }}
                      >
                        No
                      </button>
                      <button
                        type="button"
                        disabled={!hasNewFilesFromQuery}
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        style={{ padding: '2px 8px', borderRadius: '4px', border: 'none', background: 'white', color: 'black', cursor: hasNewFilesFromQuery ? 'pointer' : 'not-allowed', fontSize: '11px' }}
                      >
                        Yes
                      </button>
                    </div>
  ...
  return (
    <>
      ...
      {citationClickPanel && (() => {
        ...
        const messageCitedExcerpt = getCitedRunFromMessageText(
          citationClickPanel.sourceMessageText ?? '',
          citationClickPanel.citationNumber ?? ''
        );
        return createPortal(
          <CitationClickPanel
    ...
    <AnimatePresence mode="wait">
      {isVisible && (
        ...
        >
          ...
          >
            {/* Chat top-center toast (e.g. "No new files from this message.") */}
            {chatTopToastMessage && (
              <div
                role="status"
                aria-live="polite"
            ...
            <div 
              className="pr-4 pl-6" 
              style={{ 
                backgroundColor: showTopBlur ? 'rgba(255, 255, 255, 0.88)' : '#FFFFFF',
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setCitationPreviewClosedForMessageIds((prev) => {
                            const next = new Set(prev);
                            keysInChat.forEach((k) => next.delete(k));
                            return next;
                          });
                          setCitationReviewCompletedForMessageIds((prev) => {
                            const next = new globalThis.Set(prev);
                            keysInChat.forEach((k) => next.delete(k));
                            return next;
                          });
                          const lastWithCitations = [...chatMessages].reverse().find((m: { type?: string; text?: string }) => {
                            if (m.type === 'query') return false;
                            const text = (m as { text?: string }).text;
                            return !!(text && getOrderedCitationNumbersFromMessageText(text).length > 0);
                          });
                          if (lastWithCitations) {
                            const id = (lastWithCitations as { id?: string }).id ?? `msg-${chatMessages.indexOf(lastWithCitations)}`;
                            const ordered = getOrderedCitationNumbersFromMessageText((lastWithCitations as { text?: string }).text ?? '');
                            const savedIdx = lastClosedCitationIndexByMessageId[id];
                            const restoreIndex = typeof savedIdx === 'number' && savedIdx >= 0 && savedIdx < ordered.length ? savedIdx : 0;
                            setCitationReviewMessageId(id);
                            citationReviewMessageIdRef.current = id;
                            setCitationReviewCurrentIndex(restoreIndex);
                            setCitationReviewShowReviewNextOnly(false);
                            setCitationReviewJustRejected(false);
                          }
                        }}
            ...
            {useCenteredEmptyState ? (
              ...
              <form 
                ...
                >
                  ...
                  />
                  {/* Citation bar strip: for the single message currently "active" (in view or latest); when scrolling back to an accepted message, bar appears; when leaving, it closes again */}
                  {(() => {
                    if (!showCitationPreviewBar) return null;
                    if (!citationReviewMessageId) return null;
                    const revMsg = chatMessages.find((m) => ((m as { id?: string }).id ?? `msg-${chatMessages.indexOf(m)}`) === citationReviewMessageId);
                    if (!revMsg || !revMsg.text) return null;
                    const orderedForBar = getOrderedCitationNumbersFromMessageText(revMsg.text);
                    if (orderedForBar.length === 0) return null;
                    const reviewMsgId = (revMsg as { id?: string }).id ?? `msg-${chatMessages.indexOf(revMsg)}`;
                    const stateApplies = citationReviewMessageId === reviewMsgId;
                    const effectiveIndex = stateApplies ? citationReviewCurrentIndex : 0;
                    const effectiveShowReviewNext = stateApplies ? citationReviewShowReviewNextOnly : false;
                    const effectiveJustRejected = stateApplies ? citationReviewJustRejected : false;
                    const total = orderedForBar.length;
                    const currentNum = orderedForBar[Math.min(effectiveIndex, total - 1)];
                    const citationData = revMsg?.citations?.[currentNum];
                    const canViewDoc = !!citationData && (citationData.doc_id ?? (citationData as any).document_id);
                    const barBtn = { display: 'flex' as const, alignItems: 'center' as const, justifyContent: 'center' as const, gap: 4.4, padding: '3.3px 6.6px', fontSize: '12px', lineHeight: 1, border: '1px solid #e5e7eb', borderRadius: 5.5, cursor: 'pointer' as const, boxShadow: '0 1px 1px rgba(0,0,0,0.05)', outline: 'none' as const, minHeight: 26 };
                    return (
                      <div
                      ...
                      >
                        ...
                        >
                          <CaptionsOff size={18} strokeWidth={1.25} />
                        </button>
                          ...
                          {effectiveShowReviewNext && effectiveJustRejected ? (
                            <>
                              ...
                              <button type="button" title="Undo reject – restore this part of the response" onClick={(e) => {
                                ...
                                if (numToUndo != null) {
                                  setRejectedCitationNumbersByMessage((prev) => {
                                    const next = new globalThis.Map(prev);
                                    const set = new globalThis.Set(next.get(msgKey) ?? []);
                                    set.delete(numToUndo);
                                    if (set.size === 0) next.delete(msgKey);
                                    else next.set(msgKey, set);
                                    return next;
                                  });
                                }
                                setCitationReviewJustRejected(false);
                            ...
                            <>
                              <button type="button" title="Reject citation – remove this part from the response" onClick={(e) => {
                                ...
                                closeExpandedCardView();
                                setCitationReviewJustRejected(true);
                              ...
                              <button type="button" title="Accept Citation" onClick={(e) => {
                                ...
                                setCitationAcceptedByMessageId((prev) => {
                                  ...
                                  next[reviewMsgId] = new globalThis.Set([...existing, effectiveIndex]);
                                  return next;
                                });
                                if (effectiveIndex >= total - 1) {
                                  setCitationPreviewClosedForMessageIds((prev) => new Set(prev).add(reviewMsgId));
