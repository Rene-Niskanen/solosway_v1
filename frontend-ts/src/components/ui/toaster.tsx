    ...
    <ToastProvider duration={3000}>
      {toasts.map(function ({ id, title, description, action, duration, variant, icon, ...props }) {
        ...
        const chipLabel =
          ...
          "Error";
        const displayIcon = isSuccess ? (icon ?? <CheckCircle2 className="h-5 w-5 text-emerald-500 shrink-0" aria-hidden />) : icon;
        ...
        return (
          <Toast key={id} duration={duration || 3000} variant={variant} {...props}>
            {isDestructive ? (
              ...
              <>
                {displayIcon && <span className={cn("flex shrink-0 items-center", isSuccess && "text-emerald-500")}>{displayIcon}</span>}
                <div className={isCompact ? "grid gap-0 min-w-0" : "grid gap-1"}>
