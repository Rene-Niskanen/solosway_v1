                            ...
                            while True:
                                ...
                                # Capture node end events to update details and track state
                                    ...
                                    # Use same citation mapping as summarize path: capture chunk_citations with bbox, block_id, doc_id
                                        ...
                                        # Stream summary immediately (same as summarize_results path) - ensures formatted [1],[2],[3] is streamed
                                        if final_summary_from_responder and not summary_already_streamed:
                                            logger.info("🚀 [STREAM] IMMEDIATE: Streaming summary directly from responder (skipping checkpointer wait)")
                                            doc_count = len(chunk_citations_from_responder) if chunk_citations_from_responder else 0
                                            ...
                                            for i in range(0, len(summary_to_stream), STREAM_CHUNK_SIZE):
                                                ...
                                                if STREAM_CHUNK_DELAY_MS > 0:
                                                    time.sleep(STREAM_CHUNK_DELAY_MS / 1000.0)
                                            summary_already_streamed = True
                                            logger.info(f"🚀 [STREAM] Summary fully streamed from responder ({len(summary_to_stream)} chars)")
