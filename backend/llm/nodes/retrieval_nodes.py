...
def detect_query_characteristics(query: str) -> Dict:
    """
    Minimal query characteristics for summary_nodes compatibility.
    Returns complexity_score (0.0-1.0) and needs_comprehensive (bool).
    """
    query_lower = query.lower() if query else ""
    comprehensive_indicators = [
        "all", "every", "comprehensive", "complete", "tell me about", "describe",
        "scenario", "assumption", "period", "valuation", "market value"
    ]
    needs_comprehensive = any(ind in query_lower for ind in comprehensive_indicators)
    complexity_score = min(1.0, 0.3 if needs_comprehensive else 0.2)
    return {
        "complexity_score": complexity_score,
        "needs_comprehensive": needs_comprehensive,
        "query_type": "general",
        "expects_later_pages": needs_comprehensive,
    }
