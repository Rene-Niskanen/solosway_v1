...
- steps: array of 0, 1, or 2 steps. Each step: id, action ("retrieve_docs" or "retrieve_chunks"), query, document_ids (for retrieve_chunks: use ["<from_step_search_docs>"] when from step 1; for 1-step follow-up use the exact UUIDs from the workspace "Documents in scope" lines), reasoning_label. When you output 2 steps, step 1 MUST be retrieve_docs and step 2 MUST be retrieve_chunks. For "query": use a short phrase that reads well in the UI (e.g. "value of highlands", "Banda Lane lease summary"). The system uses the user's own message for retrieval, so include any property/document names they mentioned.
