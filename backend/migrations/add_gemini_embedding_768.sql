-- Migration: Add Gemini Embedding Support (768 dimensions)
-- Purpose: Change embedding columns and RPCs from vector(1024) to vector(768)
--          for Google Gemini embeddings (MRL-recommended dimension).
--
-- WARNING: This migration DROPS existing embeddings. Run re-embed script after applying.
--
-- Apply via Supabase SQL Editor or: psql $SUPABASE_DB_URL -f backend/migrations/add_gemini_embedding_768.sql
...
-- ============================================================================
-- STEP 2: Alter documents.document_embedding to vector(768)
-- ============================================================================

ALTER TABLE documents DROP COLUMN IF EXISTS document_embedding;
ALTER TABLE documents ADD COLUMN document_embedding vector(768);

-- ============================================================================
-- STEP 3: Alter document_vectors.embedding to vector(768)
-- ============================================================================

ALTER TABLE document_vectors DROP COLUMN IF EXISTS embedding;
ALTER TABLE document_vectors ADD COLUMN embedding vector(768);
...
-- ============================================================================
-- STEP 5: Update match_document_embeddings() to vector(768)
-- ============================================================================
...
-- ============================================================================
-- STEP 6: Update match_chunks() to vector(768)
-- ============================================================================

DROP FUNCTION IF EXISTS match_chunks(vector(1024), uuid, float, int);
...
-- ============================================================================
-- STEP 7: Update match_chunks_hybrid() to vector(768)
-- ============================================================================

DROP FUNCTION IF EXISTS match_chunks_hybrid(vector(1024), uuid, float, int, text[]);
...
-- ============================================================================
-- STEP 8: Update match_documents_hybrid() to vector(768)
-- ============================================================================
...
-- ============================================================================
-- Migration complete. Next: run re-embed script to populate embeddings.
-- ============================================================================
